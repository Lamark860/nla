<!--
  ...
  Содержит:
   ...
   - actions: избранное, share, AI-анализ
-->
<template>
  <section class="bond-hero">
    ...
    <div class="bond-hero__top">
      <div class="bond-hero__title">
        ...
        <div class="bond-hero__ids">
          ...
          <NuxtLink v-if="bond.emitter_id" :to="`/issuers/${bond.emitter_id}`" class="bond-hero__issuer">
    ...
    <!-- Tags row: rating badges, category, status, period -->
    ...
    <div class="bond-hero__kpis">
      <KPI label="Цена" :value="bond.last != null ? fmt.percent(bond.last) : '—'">
        <template #sub v-if="bond.price_rub">{{ fmt.priceRub(bond.price_rub) }}</template>
      </KPI>
      ...
      <KPI label="Купон" :value="fmt.percent(bond.coupon_display)">
        <template #sub v-if="bond.next_coupon">след. {{ fmt.dateShort(bond.next_coupon) }}</template>
      </KPI>
      ...
      <KPI label="Изменение"
           ...
           :tone="changeTone">
        <template #sub v-if="bond.value_today_rub">оборот {{ fmt.volume(bond.value_today_rub) }}</template>
      </KPI>
    </div>
  </section>
</template>
